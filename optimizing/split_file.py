...
void f???(const double *x, const double *a, double *t, double nv0) {
    #simd
    for(int i = 0; i < 8; i++) {
        t[0*8 + i] = ... ;
        t[1*8 + i] = ... ;
...
def split_file(file_in, file_out):
    ...
    for i in range(100):
        new_file.write(f"void f{i}(const double *x, const double *a, double *t, double nv0) {{")
        new_file.write("""
    size_t nv = 0;
...
""")
        ...
        new_file.write("for(size_t i = 0; nv < nd; i++) {\n")
        for l in split_lines[i]:
            new_file.write(f"{l}\n")
